...
所有关卡的网格全部填满，每支箭头至少占两格。先铺满网格，再组合相邻部分形成箭头，
高难度优先增加弯曲与折返；通过合法移除顺序及阻挡关系校验保证可解。
测试覆盖 500 关 × 4 个种子，检查完整覆盖、无单格箭头、无重叠和随机合法移除顺序。

## 选关与连续点击

顶部关卡按钮可直接选择全部 500 关，无需逐关解锁。换设备后可以手动选择原来的关卡；
进度仍保存在各设备的浏览器中，不进行云端同步。刷新后恢复上次选择的关卡。
完成标记只记录实际通过的关卡，旧版逐关进度会保留。

合法箭头点击后立即离开阻挡判定，允许连续点击其他箭头；离场动画同时播放，
全部箭头离场后再显示通关提示。
...
`arrow-level`、`arrow-progress-version`、`arrow-completed`、`arrow-sound`。
